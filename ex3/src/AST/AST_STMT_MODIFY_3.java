...
public class AST_STMT_MODIFY_3 extends AST_STMT{
    ...
    public TYPE SemantMe() {
        ...
        if (!(t instanceof TYPE_FUNCTION)) {
            System.out.println("error in EXP_MODIFY_3: not a function");
            System.exit(0);
            return null;
        }
        TYPE_FUNCTION t_func = (TYPE_FUNCTION) t;
        if (t_func.params == null) {
            System.out.println("error in EXP_MODIFY_3: should have parameters");
            System.exit(0);
            return null;
        }
        if (t_func.params.tail == null) {
            System.out.println("error in EXP_MODIFY_3: should have more then one parameter");
            System.exit(0);
            return null;
        }
        ...
        if (exp_type != t_head) {
            System.out.println("error in EXP_MODIFY_3: first parameter doesn't match");
            System.exit(0);
            return null;
        }

        TYPE_LIST l_type_list = l.GetSignatures(); //TODO: need to implement this!!!
        TYPE_LIST tmp_l = l_type_list;
        TYPE_LIST tmp_p = t_func.params.tail;
        while(tmp_l != null && tmp_p != null) {
            if (tmp_l.head != tmp_p.head)
            {
                System.out.println("error in EXP_MODIFY_3: some parameters don't match");
                System.exit(0);
                return null;
            }
            tmp_l = tmp_l.tail;
        ...
        {
            System.out.println("error in EXP_MODIFY_3: too mant parameters given for function");
            System.exit(0);
            return null;
        }
        else if (tmp_l == null && tmp_p != null)
        {
            System.out.println("error in EXP_MODIFY_3: not enough parameters given for function");
            System.exit(0);
            return null;
        }
