...
public class AST_EXP_MODIFY_6 extends AST_EXP {
                ...
                if (it.head.name.equals(id_name)) {
                    ...
                    if (class_member.type.isFunction()) {
                        ...
                        if (exp_type == null) {
                            AST_Node.file_writer.print(String.format("ERROR(%d)", this.lineNumber));
                            AST_Node.file_writer.close();
                            System.out.println(">> ERROR EXP_MODIFY_6: first parameter for function doesn't exist");
                            System.exit(0);
                            return null;
                        }
                        ...
                        while(tmp_l != null && tmp_p != null) {
                            if (tmp_l.head == null) {
                                AST_Node.file_writer.print(String.format("ERROR(%d)", this.lineNumber));
                                AST_Node.file_writer.close();
                                System.out.println(">> ERROR EXP_MODIFY_6: some parameter(second or higher) for function doesn't exist");
                                System.exit(0);
                                return null;
                            }
    ...
    public TEMP IRme() {
        ...
        IR.getInstance().Add_IRcommand(new IRcommand_Virtual_Call_Func(dst, t1, id_name, t2, lst));
        return dst;
    }
}
